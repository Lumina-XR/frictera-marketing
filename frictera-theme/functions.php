 ...
 */
function frictera_sign_gateway_request($body, $path = '/public/intake') {
    $allowed_paths = array(
        '/public/intake',
        '/public/scheduling/availability-token',
...
/**
 * Send a trusted native scheduling request to the Frictera gateway.
 *
 * Only native availability routes are permitted.
 * Secrets and raw native tokens must never be logged.
 */
function frictera_scheduling_gateway_post($path, $payload) {
    $allowed_paths = array(
        '/public/scheduling/availability-token',
        '/public/scheduling/availability',
    );
    ...
    if (!in_array($path, $allowed_paths, true)) {
        return new WP_Error(
            'invalid_scheduling_gateway_path',
            __('Scheduling request is not permitted.', 'frictera'),
            array('status' => 400)
        );
    }

    if (
        empty(FRICTERA_GATEWAY_BASE_URL) ||
        empty(FRICTERA_GATEWAY_KEY_ID) ||
        empty(FRICTERA_GATEWAY_SECRET)
    ) {
        return new WP_Error(
            'scheduling_gateway_not_configured',
            __('Scheduling is not available right now.', 'frictera'),
            array('status' => 503)
        );
    }
    ...
    if (!is_string($body)) {
        return new WP_Error(
            ...
            __('Unable to prepare scheduling request.', 'frictera'),
            array('status' => 400)
        );
    }

    $auth = frictera_sign_gateway_request($body, $path);

    if (empty($auth)) {
        return new WP_Error(
            'scheduling_auth_failed',
            __('Scheduling is not available right now.', 'frictera'),
            array('status' => 503)
        );
    }
    ...
    $url = rtrim(FRICTERA_GATEWAY_BASE_URL, '/') . $path;
    ...
    if (is_wp_error($response)) {
        return new WP_Error(
            'scheduling_gateway_unreachable',
            __('We could not load available times right now.', 'frictera'),
            array('status' => 503)
        );
    }
...
/**
 * Issue a native Frictera availability token for a validated intake.
 */
function frictera_issue_native_availability_token($intake_reference) {
    $intake_reference = sanitize_text_field($intake_reference);
    ...
    if (
        ...
        return new WP_Error(
            ...
            __('Unable to load available times.', 'frictera'),
            array('status' => 400)
        );
    }

    $result = frictera_scheduling_gateway_post(
        '/public/scheduling/availability-token',
        array(
    ...
    if (
        intval($result['status']) !== 200 ||
        empty($result['body']['token']) ||
        empty($result['body']['intakeReference']) ||
        empty($result['body']['eventReference'])
    ) {
    ...
    return array(
        'token'           => sanitize_text_field($result['body']['token']),
        'intakeReference' => sanitize_text_field($result['body']['intakeReference']),
        'eventReference'  => sanitize_text_field($result['body']['eventReference']),
        'expiresAt'       => sanitize_text_field($result['body']['expiresAt'] ?? ''),
    );
}
/**
...
/**
 * Build a bounded meeting-purpose snapshot from canonical friction signals.
 */
function frictera_calcom_title_from_frictions($frictions) {
    $labels = array(
        'slow-enquiry-response'  => 'Slow response to new enquiries',
        'lead-routing-delay'     => 'Leads not reaching the right person quickly enough',
        'missing-information'    => 'Enquiries arrive with missing information',
        'missed-follow-up'       => 'Follow-ups are being missed or delayed',
        'attention-visibility'   => 'Difficult to see which enquiries need attention',
        'manual-channel-handoff' => 'Too much manual work between portals, email, WhatsApp and CRM',
        'pm-coordination-delay'  => 'Property-management issues take too long to coordinate',
        'landlord-approval-delay'=> 'Landlord approvals are delayed',
        'maintenance-tracking'   => 'Maintenance requests are difficult to track',
        'management-visibility'  => 'Lack clear management visibility',
        'automation-discovery'   => 'Want to identify where automation would genuinely help',
    );
    $parts = array();

    foreach (is_array($frictions) ? $frictions : array() as $friction) {
        if (!is_array($friction) || ($friction['signalType'] ?? '') !== 'friction') {
            continue;
        }

        $code = sanitize_text_field($friction['signalCode'] ?? '');
        if (isset($labels[$code])) {
            $parts[] = $labels[$code];
        } elseif ($code === 'other' && !empty($friction['otherText'])) {
            $parts[] = sanitize_text_field($friction['otherText']);
        }
    }

    $title = sanitize_text_field(implode('; ', $parts));
    $title = trim(preg_replace('/\s+/', ' ', $title));
    if ($title === '') {
        return new WP_Error('empty_scheduling_title', 'A meeting purpose is required.');
    }
    if (strlen($title) > 500) {
        return new WP_Error('scheduling_title_too_long', 'The meeting purpose exceeds 500 characters.');
    }
 ...
 */
function frictera_create_scheduling_token($submission_id, $title = '') {
    if (empty(FRICTERA_SCHEDULING_TOKEN_SECRET) || strlen(FRICTERA_SCHEDULING_TOKEN_SECRET) < 32) {
        return '';
    }
    $title = trim(preg_replace('/\s+/', ' ', sanitize_text_field($title)));
...
function frictera_verify_scheduling_token($token) {
    ...
    if (empty($payload) || empty($payload['exp']) || empty($payload['sub']) || empty($payload['title'])) {
        return null;
    }
    if (intval($payload['exp']) < time()) {
        return null;
    }
    $title = trim(preg_replace('/\s+/', ' ', sanitize_text_field($payload['title'])));
    if ($title === '' || strlen($title) > 500) {
        return null;
    }
    return array(
        'submission_id' => sanitize_text_field($payload['sub']),
        'title'         => $title,
    );
}
...
/**
 * Map Cal.com v2 endpoints to their required API versions.
 *
 * Cal.com v2 requires endpoint-specific cal-api-version headers. Passing the
 * wrong version causes the endpoint to fall back to an older/unsupported
 * contract. The version values are taken from the authoritative Cal.com v2
 * OpenAPI documentation.
 */
function frictera_calcom_api_version($path) {
    if (strpos($path, '/v2/bookings') === 0) {
        return '2026-02-25';
    }
    if (strpos($path, '/v2/event-types') === 0) {
        return '2024-06-14';
    }
    if (strpos($path, '/v2/slots') === 0) {
        return '2024-09-04';
    }
    return '2024-06-11';
}
...
function frictera_calcom_request($path, $method = 'GET', $body = null) {
    ...
    $headers = array(
        ...
        'Content-Type'    => 'application/json',
        'cal-api-version' => frictera_calcom_api_version($path),
    );
 ...
 *
 * Prefers a configured numeric ID. Otherwise queries the current Cal.com v2
 * event-types endpoint using the documented username/eventSlug selector.
 */
function frictera_calcom_event_type_id() {
    ...
    $query = http_build_query(array(
        'username'  => FRICTERA_CALCOM_USERNAME,
        'eventSlug' => FRICTERA_CALCOM_EVENT_TYPE_SLUG,
    ));
    $response = frictera_calcom_request('/v2/event-types?' . $query);
...
/**
 * Normalize a Cal.com slot start to the UTC format required by bookings v2.
 */
function frictera_calcom_booking_start($slot_time) {
    $timestamp = strtotime($slot_time);
    if ($timestamp === false) {
        return '';
    }
...
function frictera_rest_scheduling_slots($request) {
    ...
    $intake_reference = sanitize_text_field(
        $params['intakeReference'] ??
        $request->get_param('intakeReference') ??
        ''
    );
    ...
    $native_token = frictera_issue_native_availability_token($intake_reference);

    if (is_wp_error($native_token)) {
        $data = $native_token->get_error_data();
        ...
        return new WP_REST_Response(array(
            ...
            'slots'     => array(),
            'message'   => $status === 429
                ? 'Too many scheduling requests. Please try again shortly.'
                : 'Scheduling is not available right now. We will be in touch by email.',
        ), $status);
    }
    ...
    $availability_payload = array(
        'token'           => $native_token['token'],
        'intakeReference' => $native_token['intakeReference'],
        'eventReference'  => 'friction-review-20',
        'timezone'        => $timezone,
        'startDate'       => $now->format(DateTimeInterface::RFC3339),
        'endDate'         => $end->format(DateTimeInterface::RFC3339),
    );
    ...
    unset($availability_payload['token']);
    unset($native_token['token']);
    ...
    if ($status === 400) {
        return new WP_REST_Response(array(
            'available' => false,
            'slots'     => array(),
            'message'   => 'Unable to load available times. Please check your request.',
        ), 400);
    }
    ...
    if ($status === 429) {
        return new WP_REST_Response(array(
            'available' => false,
            'slots'     => array(),
            'message'   => 'Too many scheduling requests. Please try again shortly.',
        ), 429);
    }
    ...
    $native_slots = isset($body['slots']) && is_array($body['slots'])
        ? $body['slots']
        : array();
...
function frictera_rest_scheduling_book($request) {
    ...
    $booking_start = frictera_calcom_booking_start($slot_time);
    ...
    $booking_body = array(
        ...
        'guests'      => array(),
        'bookingFieldsResponses' => array(
            'title' => sanitize_text_field($verified['title']),
        ),
        'metadata'    => array(
    ...
    $start_time = isset($response['data']['start']) ? sanitize_text_field($response['data']['start']) : $booking_start;
 ...
 *
 * Cal.com signs webhook payloads with HMAC-SHA256 over the raw request body
 * using a subscriber-supplied secret and sends the result in the
 * X-Cal-Signature-256 header. Evidence: Cal.com source package
 * packages/features/webhooks/lib/sendPayload.ts (createWebhookSignature).
 *
 * This handler also accepts the legacy x_cal_signature_v2 header name for
 * backward compatibility with older provider payloads.
 */
function frictera_rest_scheduling_webhook($request) {
    ...
    $signature = '';
    if (isset($headers['x_cal_signature_256'])) {
        $signature = sanitize_text_field($headers['x_cal_signature_256']);
    } elseif (isset($headers['x-cal-signature-256'])) {
        $signature = sanitize_text_field($headers['x-cal-signature-256']);
    } elseif (isset($headers['x_cal_signature_v2'])) {
        $signature = sanitize_text_field($headers['x_cal_signature_v2']);
    }
...
function frictera_build_contact_response($result) {
    ...
    if ($data['schedulingEligible']) {
        /*
         * Native scheduling authority is derived server-side from the
         * validated Frictera intake reference. Do not create or expose
         * the legacy WordPress/Cal.com scheduling token here.
         */
        $data['schedulingIntakeReference'] = sanitize_text_field($data['submissionId']);
